...
<script>
    layui.use(['form','jquery','layer'], function () {
        ...
        form.on('submit(login)', function (data) {
            ...
            $.ajax({
                  url:'/ingredient/login.a',//发出请求
                  type:'post',
                  data:{"jb_text":data.username,"jb_password":data.password},
                  success:function(data){
                       console.log(data);
                       if(data=="ok"){//ok说明登录成功
                           layer.msg('登录成功', function () {
                                           window.location = '/ingredient/index.jsp';});
                             //window.location.href="UserServlet?action=toLogin";然后跳转到你系统的主界面
                        }else if(data=="err"){
                            layer.msg('登录失败,用户民或密码错误');
                           / //$("#msg").html("用户名或密码错误!");
                             //setTimeout(function(){ $("#msg").html("")},2000);//定时器，3秒钟后提示消失       
                        }
                    }
             });
