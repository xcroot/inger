        ...
        <table class="layui-table" id="currentTableId"  lay-filter="currentTableFilter">
           <thead>
           		<tr>
           		   ...
           		   <th lay-data="{field:'relatedPe', width:180}">对应工程师</th>
           		   <th lay-data="{field:'materialProvider', width:180}">供应商</th>
           		   <th lay-data="{field:'operator', width:180}">申请人</th>
           		   <th lay-data="{field:'isChecked', width:180}">是否需要校验</th>
           ...
           <tbody>
			<%
					...
					List<Mach> items = (List<Mach>)request.getAttribute("machorderall");
					for(int i=0;i<items.size();i++){
						Mach item = items.get(i);
						%>
						<tr>
							<td>
								<%=item.getPurchaseId()%>
							...
							<td>
								<%=item.getIsChecked()== null ? "":item.getIsChecked()%>
							</td>
    ...
    layui.use(['form', 'table','layuimini','element','laydate'], function () {
        ...
        form.on('submit(js-search-filter)', function (data) {
            table.reload('currentTableId',{
             method:'post',
             url:'ruike/ordermachsearchbydate.do?&'+'startdate='+data.field.startdate+'&'+'enddate='+data.field.enddate
             });
            ...
            if (obj.event === 'edit') {
                ...
                layer.confirm('确定取消该请购吗?', function (index) {
                    console.log(data.purchaseId)
                     $.ajax({
                          url:'/ingredient/ruike/machorderquit.do',//发出请求
                          type:'post',
