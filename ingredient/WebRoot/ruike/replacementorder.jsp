        ...
        <table class="layui-table" id="currentTableId"  lay-filter="currentTableFilter">
           <thead>
           		<tr>
           		   ...
           		   <th lay-data="{field:'materialProvider', width:180}">厂家</th>
           		   <th lay-data="{field:'equipmentUse', width:180}">所属设备</th>
           		   <th lay-data="{field:'reason', width:180}">申购原因</th>
          ...
          <tbody>
				<%
						List<Replacement> items = (List<Replacement>)request.getAttribute("replacementorderall");
						for(int i=0;i<items.size();i++){
							Replacement item = items.get(i);
							%>
							<tr>
								<td>
									<%=item.getPurchaseId()%>
								...
								<td>
									<%=item.getMaterialProvider()%>
								</td>
								<td>
									<%=item.getEquipmentUse()%>
								</td>
								<td>
									<%=item.getReason()%>
								</td>
    ...
    layui.use(['form', 'table','layuimini','element','laydate'], function () {
        ...
        form.on('submit(js-search-filter)', function (data) {
            table.reload('currentTableId',{
             method:'post',
             url:'ruike/orderreplacementsearchbydate.do?&'+'startdate='+data.field.startdate+'&'+'enddate='+data.field.enddate
             });
            ...
            if (obj.event === 'edit') {
                ...
                layer.confirm('确定取消该请购吗?', function (index) {
                    console.log(data.purchaseId)
                     $.ajax({
                          url:'/ingredient/ruike/replacementorderquit.do',//发出请求
                          type:'post',
