        ...
        <table class="layui-table" id="currentTableId"  lay-filter="currentTableFilter">
           <thead>
           		<tr>
           		   ...
           		   <th lay-data="{field:'addr', width:180}">收货地址</th>
           		   <th lay-data="{field:'deathLine', width:180}">交货日期</th>
          ...
          <tbody>
          <%					
				List<EazyConsume> items = (List<EazyConsume>)request.getAttribute("eazyconsumeorderall");
					for(int i=0;i<items.size();i++){
						EazyConsume item = items.get(i);
						%>
						<tr>
							<td>
								<%=item.getPurchaseId()%>
							...
							<td>
								<%=item.getAddr()== null ? "":item.getAddr()%>
							</td>
							<td>
								<%=item.getDeathLine()== null ? "":item.getDeathLine()%>
							</td>
							<td>
